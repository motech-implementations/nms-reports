...
package final_etl.campaign_schedule_0_1;
...
/**
 * Job: Campaign_Schedule Purpose: Campaign_Schedule<br>
 * Description: Campaign_Schedule <br>
 * @author user@talend.com
 * @version 6.1.1.20151214_1327
 * @status 
 */
public class Campaign_Schedule implements TalendJob {
	...
	private final String jobName = "Campaign_Schedule";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Campaign_Schedule.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Campaign_Schedule.this, new Object[] { e,
	...
	public static class row3Struct implements
		...
		final static byte[] commonByteArrayLock_FINAL_ETL_Campaign_Schedule = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_Campaign_Schedule = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_FINAL_ETL_Campaign_Schedule.length) {
					if (length < 1024
							&& commonByteArray_FINAL_ETL_Campaign_Schedule.length == 0) {
						commonByteArray_FINAL_ETL_Campaign_Schedule = new byte[1024];
					} else {
						commonByteArray_FINAL_ETL_Campaign_Schedule = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_FINAL_ETL_Campaign_Schedule, 0,
						length);
				strReturn = new String(
						commonByteArray_FINAL_ETL_Campaign_Schedule, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_Campaign_Schedule) {
	...
	public static class insert_camapign_schedule_dataStruct
			implements
			routines.system.IPersistableRow<insert_camapign_schedule_dataStruct> {
		final static byte[] commonByteArrayLock_FINAL_ETL_Campaign_Schedule = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_Campaign_Schedule = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final insert_camapign_schedule_dataStruct other = (insert_camapign_schedule_dataStruct) obj;
		...
		public void copyDataTo(insert_camapign_schedule_dataStruct other) {
		...
		public void copyKeysDataTo(insert_camapign_schedule_dataStruct other) {
		 ...
		 * Compare keys
		 */
		public int compareTo(insert_camapign_schedule_dataStruct other) {
	...
	public static class row1Struct implements
		...
		public int messagesPerWeek;

		public int getMessagesPerWeek() {
			return this.messagesPerWeek;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_Campaign_Schedule) {
				...
				try {
					...
					this.messagesPerWeek = dis.readInt();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeInt(this.messagesPerWeek);
		...
		public String toString() {
			...
			sb.append(",messagesPerWeek=" + String.valueOf(messagesPerWeek));
			sb.append(",Subscription_Start_Date="
					+ String.valueOf(Subscription_Start_Date));
			sb.append(",modificationDate=" + modificationDate);
			sb.append("]");
	...
	public void tMysqlInput_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				insert_camapign_schedule_dataStruct insert_camapign_schedule_data = new insert_camapign_schedule_dataStruct();
				...
				String tableName_tMysqlOutput_1 = "campaign_schedule_alerts";
				...
				String insert_tMysqlOutput_1 = "INSERT INTO `"
						+ "campaign_schedule_alerts"
						+ "` (`Subscription_ID`,`Campaign_ID`,`Date_ID`,`Time_ID`,`Subscription_Start_Date`,`modificationDate`) VALUES (?,?,?,?,?,?)";
				...
				insert_camapign_schedule_dataStruct insert_camapign_schedule_data_tmp = new insert_camapign_schedule_dataStruct();
				// ###############################
				...
				String dbquery_tMysqlInput_1 = "SELECT * FROM(SELECT gp.Subscription_Id,gp.weekId,gp.Subscription_Pack_Name,\ngp.creationDate,gp.messagesPerWeek,gp.Subscription_Start_Date\nFROM (SELECT sbs.id as 'Subscription_Id',csr.weekId,\nsp.name as 'Subscription_Pack_Name',\nSTR_TO_DATE(SUBSTRING_INDEX(csr.requestId, ':', 1),'%Y%m%d%H%i%S') as 'creationDate',\nMOD(TIMESTAMPDIFF(DAY, DATE(sbs.startDate), DATE(STR_TO_DATE(SUBSTRING_INDEX(csr.requestId, ':', 1),'%Y%m%d%H%i%S'))),7) AS 'Mod',\nsp.messagesPerWeek,\nsbs.startDate as 'Subscription_Start_Date'\nFROM nms_imi_csrs csr\nInner join nms_subscriptions sbs  on SUBSTRING_INDEX(csr.requestId, ':', -1)=sbs.subscriptionId\nInner join nms_subscription_packs sp on sbs.subscriptionPack_id_OID=sp.id\nWHERE csr.creationDate>'"
						+ (String) globalMap.get("lastEtlTime")
						+ "'\norder by sbs.id,csr.weekId,csr.creationDate asc )\ngp \nWHERE ((gp.messagesPerWeek=1 AND gp.Mod=0) ||(gp.messagesPerWeek=2 AND gp.Mod in(0,4))))\nmain CROSS JOIN (SELECT MAX(modificationDATE) AS modificationDate FROM nms_imi_csrs)csr2";
				...
				try {
					...
					while (rs_tMysqlInput_1.next()) {
						...
						if (colQtyInRs_tMysqlInput_1 < 2) {
							...
							row1.weekId = routines.system.JDBCUtil.getString(
						...
						if (colQtyInRs_tMysqlInput_1 < 5) {
							row1.messagesPerWeek = 0;
						} else {

							if (rs_tMysqlInput_1.getObject(5) != null) {
								row1.messagesPerWeek = rs_tMysqlInput_1
										.getInt(5);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
						...
						if (colQtyInRs_tMysqlInput_1 < 7) {
							...
							row1.modificationDate = routines.system.JDBCUtil
									.getString(rs_tMysqlInput_1, 7, false);
						}
						...
						{ // start of Var scope
							...
							insert_camapign_schedule_data = null;

							// # Output table : 'insert_camapign_schedule_data'
							insert_camapign_schedule_data_tmp.Subscription_ID = row1.Subscription_Id;
							insert_camapign_schedule_data_tmp.Campaign_ID = ReportingHelper
									.getCampaignId(row1.weekId,
											row1.Subscription_Pack_Name,
											row1.messagesPerWeek,
											(java.sql.Connection) resourceMap
													.get("conn_tMysqlOutput_1"));
							insert_camapign_schedule_data_tmp.Date_ID = ReportingHelper
									.getDateId(row1.creationDate,
											(java.sql.Connection) resourceMap
													.get("conn_tMysqlOutput_1"));
							insert_camapign_schedule_data_tmp.Time_ID = 0;
							insert_camapign_schedule_data_tmp.Subscription_Start_Date = row1.Subscription_Start_Date;
							insert_camapign_schedule_data_tmp.modificationDate = row1.modificationDate;
							insert_camapign_schedule_data = insert_camapign_schedule_data_tmp;
							// ###############################
						 ...
						 * [tMap_1 main ] stop
						 */
						// Start of branch "insert_camapign_schedule_data"
						if (insert_camapign_schedule_data != null) {
							...
							pstmt_tMysqlOutput_1
									.setLong(
											1,
											insert_camapign_schedule_data.Subscription_ID);

							pstmt_tMysqlOutput_1.setLong(2,
									insert_camapign_schedule_data.Campaign_ID);

							pstmt_tMysqlOutput_1.setInt(3,
									insert_camapign_schedule_data.Date_ID);

							pstmt_tMysqlOutput_1.setInt(4,
									insert_camapign_schedule_data.Time_ID);

							if (insert_camapign_schedule_data.Subscription_Start_Date != null) {
								date_tMysqlOutput_1 = insert_camapign_schedule_data.Subscription_Start_Date
										.getTime();
							...
							if (insert_camapign_schedule_data.modificationDate == null) {
								...
								pstmt_tMysqlOutput_1
										.setString(
												6,
												insert_camapign_schedule_data.modificationDate);
							}
						...
						} // End of branch "insert_camapign_schedule_data"
	...
	public void tMysqlRow_1Process(final java.util.Map<String, Object> globalMap)
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				query_tMysqlRow_1 = "UPDATE ETL_info_table SET ETL_info_table.last_etl_time = (SELECT MAX(modificationDate) from campaign_schedule_alerts),\nETL_info_table.record_count=(SELECT COUNT(*) from campaign_schedule_alerts)\nWHERE ETL_info_table.table_id = 21";
	...
	public static void main(String[] args) {
		final Campaign_Schedule Campaign_ScheduleClass = new Campaign_Schedule();

		int exitCode = Campaign_ScheduleClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Campaign_Schedule.class
					.getClassLoader().getResourceAsStream(
							"final_etl/campaign_schedule_0_1/contexts/"
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : Campaign_Schedule");
		}
...
/************************************************************************************************
 * 86649 characters generated by Talend Open Studio for Big Data on the May 20,
 * 2016 2:50:05 PM PDT
 ************************************************************************************************/
