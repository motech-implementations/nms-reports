...
package final_etl.time_dimension_0_1;
...
/**
 * Job: time_dimension Purpose: time_dimension reporting table<br>
 * Description: time_dimension reporting table generation <br>
 * @author user@talend.com
 * @version 6.1.1.20151214_1327
 * @status DEV
 */
public class time_dimension implements TalendJob {
	...
	private final String jobName = "time_dimension";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					time_dimension.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(time_dimension.this, new Object[] { e,
	...
	public static class row3Struct implements
		...
		final static byte[] commonByteArrayLock_FINAL_ETL_time_dimension = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_time_dimension = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_FINAL_ETL_time_dimension.length) {
					if (length < 1024
							&& commonByteArray_FINAL_ETL_time_dimension.length == 0) {
						commonByteArray_FINAL_ETL_time_dimension = new byte[1024];
					} else {
						commonByteArray_FINAL_ETL_time_dimension = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_FINAL_ETL_time_dimension, 0,
						length);
				strReturn = new String(
						commonByteArray_FINAL_ETL_time_dimension, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_time_dimension) {
	...
	public static class time_dataStruct implements
			routines.system.IPersistableRow<time_dataStruct> {
		final static byte[] commonByteArrayLock_FINAL_ETL_time_dimension = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_time_dimension = new byte[0];
		...
		public java.util.Date FullTime;
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final time_dataStruct other = (time_dataStruct) obj;
		...
		public void copyDataTo(time_dataStruct other) {

			other.ID = this.ID;
			other.Hour_Of_Day = this.Hour_Of_Day;
			other.Minute_Of_Hour = this.Minute_Of_Hour;
			other.FullTime = this.FullTime;

		}

		public void copyKeysDataTo(time_dataStruct other) {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_time_dimension) {
				...
				try {
					...
					this.Hour_Of_Day = dis.readInt();

					this.Minute_Of_Hour = dis.readInt();

					this.FullTime = readDate(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeInt(this.Hour_Of_Day);

				// int

				dos.writeInt(this.Minute_Of_Hour);

				// java.util.Date

				writeDate(this.FullTime, dos);
		...
		public String toString() {
			...
			sb.append(",Hour_Of_Day=" + String.valueOf(Hour_Of_Day));
			sb.append(",Minute_Of_Hour=" + String.valueOf(Minute_Of_Hour));
			sb.append(",FullTime=" + String.valueOf(FullTime));
			sb.append("]");
		 ...
		 * Compare keys
		 */
		public int compareTo(time_dataStruct other) {
	...
	public static class row1Struct implements
			...
			synchronized (commonByteArrayLock_FINAL_ETL_time_dimension) {
				...
				try {
					...
					if (length == -1) {
						...
						this.pkey = dis.readLong();
					}

					this.ID = dis.readInt();

					this.AMPM = readString(dis);
		...
		public String toString() {
			...
			sb.append(",Minute_Of_Hour=" + String.valueOf(Minute_Of_Hour));
			sb.append(",FullTime=" + String.valueOf(FullTime));
			sb.append(",pkey=" + String.valueOf(pkey));
			sb.append(",ID=" + String.valueOf(ID));
			sb.append(",AMPM=" + AMPM);
			sb.append("]");
	...
	public void tMysqlInput_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				time_dataStruct time_data = new time_dataStruct();
				...
				String tableName_tMysqlOutput_1 = "time_dimension";
				...
				java.sql.PreparedStatement pstmt_tMysqlOutput_1 = conn_tMysqlOutput_1
						.prepareStatement("SELECT COUNT(1) FROM `"
								+ "time_dimension" + "` WHERE `ID` = ?");
				String insert_tMysqlOutput_1 = "INSERT INTO `"
						+ "time_dimension"
						+ "` (`ID`,`Hour_Of_Day`,`Minute_Of_Hour`,`FullTime`) VALUES (?,?,?,?)";
				...
				String update_tMysqlOutput_1 = "UPDATE `"
						+ "time_dimension"
						+ "` SET `Hour_Of_Day` = ?,`Minute_Of_Hour` = ?,`FullTime` = ? WHERE `ID` = ?";
				...
				time_dataStruct time_data_tmp = new time_dataStruct();
				// ###############################
				...
				String dbquery_tMysqlInput_1 = "select tim.hou as Hour_Of_Day, tim.min as Minutes_Of_Hour, TIME_FORMAT(CONCAT(tim.hou,':',tim.min),'%H:%i:%s') as FullTime, tim.pkey as pkey\nFROM\n(select a.pkey,a.ht,\n          (CASE (a.ht%60) WHEN 0 THEN @m:=0 ELSE @m:=@m+1 END)  as min,\n          (CASE WHEN (a.ht > 0) THEN (CASE (a.ht%60) WHEN 0 THEN @h:=@h+1 ELSE @h:=@h END) ELSE @h:=0 END)  as hou \n      from (\n           select date1.pkey as pkey , (a.a + (10 * b.a) + (100*c.a) + (1000*d.a)) as ht\n            from \n              (select record_count as pkey  from ETL_info_table where ETL_info_table.table_id=16) as date1,\n              (select 0 as a union all select 1 union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9) as a\n                cross join (select 0 as a union all select 1 union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9) as b\n                cross join (select 0 as a union all select 1 union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9) as c\n                cross join (select 0 as a union all select 1 union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9) as d\n              ) a,\n              (SELECT @h:= 0) r1, (SELECT @m:= 0) r2\n  limit 1440)tim\n  ";
					...
					while (rs_tMysqlInput_1.next()) {
						...
						if (colQtyInRs_tMysqlInput_1 < 1) {
							...
							if (rs_tMysqlInput_1.getObject(1) != null) {
								row1.Hour_Of_Day = rs_tMysqlInput_1.getInt(1);
							} else {
								throw new RuntimeException(
						...
						if (colQtyInRs_tMysqlInput_1 < 2) {
							row1.Minute_Of_Hour = 0;
						} else {

							if (rs_tMysqlInput_1.getObject(2) != null) {
								row1.Minute_Of_Hour = rs_tMysqlInput_1
										.getInt(2);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
						...
						if (colQtyInRs_tMysqlInput_1 < 3) {
							...
							if (rs_tMysqlInput_1.getString(3) != null) {
								...
								if (!("0000-00-00")
										...
										&& !("0000-00-00 00:00:00")
												.equals(dateString_tMysqlInput_1)) {
									row1.FullTime = rs_tMysqlInput_1
											.getTimestamp(3);
								} else {
									row1.FullTime = (java.util.Date) year0_tMysqlInput_1
											.clone();
								}
							} else {
								row1.FullTime = null;
						...
						if (colQtyInRs_tMysqlInput_1 < 4) {
							...
							if (rs_tMysqlInput_1.getObject(4) != null) {
								row1.pkey = rs_tMysqlInput_1.getLong(4);
							} else {
								row1.pkey = null;
							}
						}
						if (colQtyInRs_tMysqlInput_1 < 5) {
							...
							if (rs_tMysqlInput_1.getObject(5) != null) {
								row1.ID = rs_tMysqlInput_1.getInt(5);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
							}
						...
						if (colQtyInRs_tMysqlInput_1 < 6) {
							...
							row1.AMPM = routines.system.JDBCUtil.getString(
						...
						row1.pkey == 0
							...
							{ // start of Var scope
								...
								// # Output table : 'time_data'
								time_data_tmp.ID = row1.ID;
								time_data_tmp.Hour_Of_Day = row1.Hour_Of_Day;
								time_data_tmp.Minute_Of_Hour = row1.Minute_Of_Hour;
								time_data_tmp.FullTime = row1.FullTime;
								time_data = time_data_tmp;
								// ###############################
							...
							if (time_data != null) {
								...
								pstmt_tMysqlOutput_1.setInt(1, time_data.ID);
								...
								if (checkCount_tMysqlOutput_1 > 0) {
									pstmtUpdate_tMysqlOutput_1.setInt(1,
											time_data.Hour_Of_Day);

									pstmtUpdate_tMysqlOutput_1.setInt(2,
											time_data.Minute_Of_Hour);

									if (time_data.FullTime != null) {
										date_tMysqlOutput_1 = time_data.FullTime
												.getTime();
	...
	public static void main(String[] args) {
		final time_dimension time_dimensionClass = new time_dimension();

		int exitCode = time_dimensionClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = time_dimension.class
					.getClassLoader().getResourceAsStream(
							"final_etl/time_dimension_0_1/contexts/"
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : time_dimension");
		}
...
/************************************************************************************************
 * 75130 characters generated by Talend Open Studio for Big Data on the May 17,
 * 2016 4:00:45 PM PDT
 ************************************************************************************************/
