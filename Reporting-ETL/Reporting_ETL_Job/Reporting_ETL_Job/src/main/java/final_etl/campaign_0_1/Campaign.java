...
package final_etl.campaign_0_1;
...
/**
 * Job: Campaign Purpose: campaign_dimension reporting table<br>
 * Description: campaign_dimension reporting table generated from motech data services <br>
 * @author user@talend.com
 * @version 6.1.1.20151214_1327
 * @status DEV
 */
public class Campaign implements TalendJob {
	...
	private final String jobName = "Campaign";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Campaign.this, new Object[] { e,
	...
	public static class row3Struct implements
		...
		final static byte[] commonByteArrayLock_FINAL_ETL_Campaign = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_Campaign = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_FINAL_ETL_Campaign.length) {
					if (length < 1024
							&& commonByteArray_FINAL_ETL_Campaign.length == 0) {
						commonByteArray_FINAL_ETL_Campaign = new byte[1024];
					} else {
						commonByteArray_FINAL_ETL_Campaign = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_FINAL_ETL_Campaign, 0, length);
				strReturn = new String(commonByteArray_FINAL_ETL_Campaign, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_Campaign) {
	...
	public static class campaign_dataStruct implements
			routines.system.IPersistableRow<campaign_dataStruct> {
		final static byte[] commonByteArrayLock_FINAL_ETL_Campaign = new byte[0];
		static byte[] commonByteArray_FINAL_ETL_Campaign = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final campaign_dataStruct other = (campaign_dataStruct) obj;
		...
		public void copyDataTo(campaign_dataStruct other) {

			other.ID = this.ID;
			other.Campaign_ID = this.Campaign_ID;
			other.Message_Duration = this.Message_Duration;
			other.modificationDate = this.modificationDate;

		}

		public void copyKeysDataTo(campaign_dataStruct other) {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_FINAL_ETL_Campaign) {
				...
				try {
					...
					this.Message_Duration = dis.readInt();
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeInt(this.Message_Duration);
		...
		public String toString() {
			...
			sb.append(",Message_Duration=" + String.valueOf(Message_Duration));
		 ...
		 * Compare keys
		 */
		public int compareTo(campaign_dataStruct other) {
	...
	public void tMysqlInput_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				campaign_dataStruct campaign_data = new campaign_dataStruct();
				...
				String tableName_tMysqlOutput_1 = "campaign_dimension";
				...
				java.sql.PreparedStatement pstmt_tMysqlOutput_1 = conn_tMysqlOutput_1
						.prepareStatement("SELECT COUNT(1) FROM `"
								+ "campaign_dimension" + "` WHERE `ID` = ?");
				String insert_tMysqlOutput_1 = "INSERT INTO `"
						+ "campaign_dimension"
						+ "` (`ID`,`Campaign_ID`,`Message_Duration`,`modificationDate`) VALUES (?,?,?,?)";
				...
				String update_tMysqlOutput_1 = "UPDATE `"
						+ "campaign_dimension"
						+ "` SET `Campaign_ID` = ?,`Message_Duration` = ?,`modificationDate` = ? WHERE `ID` = ?";
				...
				campaign_dataStruct campaign_data_tmp = new campaign_dataStruct();
				// ###############################
				...
				String dbquery_tMysqlInput_1 = "select nms_subscription_pack_messages.id, weekId, duration,nms_subscription_pack_messages.modificationDate\nfrom nms_subscription_pack_messages\nINNER JOIN nms_subscription_packs\nON nms_subscription_packs.id = nms_subscription_pack_messages.messages_id_OWN &&\nnms_subscription_packs.name = '72WeeksPack' \nWHERE nms_subscription_pack_messages.creationDate>'"
						+ (String) globalMap.get("lastEtlTime")
						+ "'\nOR nms_subscription_pack_messages.modificationDate >'"
						+ (String) globalMap.get("lastEtlTime") + "'";
					...
					while (rs_tMysqlInput_1.next()) {
						...
						if (colQtyInRs_tMysqlInput_1 < 3) {
							...
							if (rs_tMysqlInput_1.getObject(3) != null) {
								row1.duration = rs_tMysqlInput_1.getInt(3);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
							}
						...
						{ // start of Var scope
							...
							// # Output table : 'campaign_data'
							campaign_data_tmp.ID = row1.id;
							campaign_data_tmp.Campaign_ID = row1.weekId;
							campaign_data_tmp.Message_Duration = row1.duration;
							campaign_data_tmp.modificationDate = row1.modificationDate;
							campaign_data = campaign_data_tmp;
							// ###############################
						...
						if (campaign_data != null) {
							...
							pstmt_tMysqlOutput_1.setLong(1, campaign_data.ID);
							...
							if (checkCount_tMysqlOutput_1 > 0) {
								if (campaign_data.Campaign_ID == null) {
								...
								pstmtUpdate_tMysqlOutput_1.setInt(2,
										campaign_data.Message_Duration);

								if (campaign_data.modificationDate == null) {
									pstmtUpdate_tMysqlOutput_1.setNull(3,
											java.sql.Types.VARCHAR);
								} else {
									pstmtUpdate_tMysqlOutput_1.setString(3,
											campaign_data.modificationDate);
								}
	...
	public void tMysqlRow_1Process(final java.util.Map<String, Object> globalMap)
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				query_tMysqlRow_1 = "UPDATE ETL_info_table SET ETL_info_table.last_etl_time = (SELECT max(modificationDate) from campaign_dimension),\nETL_info_table.record_count=(SELECT COUNT(*) from campaign_dimension)\nWHERE ETL_info_table.table_id = 10";
	...
	public static void main(String[] args) {
		final Campaign CampaignClass = new Campaign();

		int exitCode = CampaignClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Campaign.class.getClassLoader()
					.getResourceAsStream(
							"final_etl/campaign_0_1/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : Campaign");
		}
...
/************************************************************************************************
 * 81260 characters generated by Talend Open Studio for Big Data on the May 17,
 * 2016 4:00:47 PM PDT
 ************************************************************************************************/
